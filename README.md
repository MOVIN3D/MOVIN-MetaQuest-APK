...
패널은 앱 실행 시 정면에 자동 배치됩니다. 자세를 바꾸거나 몸이 돌아가서 패널이 시야를 벗어났을 때:

- **수동** — 컨트롤러의 **B 버튼**(오른손) 또는 **Y 버튼**(왼손)을 0.3초 이상 누르면 즉시 정면으로 이동합니다.
- **자동** — 패널이 정면 시야 콘에서 1.5초 이상 벗어나 있으면 알아서 다시 정면으로 옮겨집니다.
